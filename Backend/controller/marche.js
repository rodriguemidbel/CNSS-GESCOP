...
class MarcheController {
  ...
  async findMarche(req, res) {
    try {
      const {dossier_id} = req.params;
      const item = await marcheService.findMarche(dossier_id);
      if(item)
      {
  ...
  /*-----*/
  async countMarche(req, res) {
    try {
      const {annee} = req.params;
      const item = await marcheService.countMarche(annee);
      if(item)
      {
        res.status(201).json(item);
      }
  ...
  async getMarche(req, res) {
    try {
      const {dossier_id} = req.params;
      const item = await marcheService.getMarche(dossier_id);
      if(item)
      {
        res.status(201).json(item);
      }
