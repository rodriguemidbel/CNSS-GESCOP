...
class ReceptionController {
  ...
  async findReception(req, res) {
    try {
      const {marche_id} = req.params;
     
      const Receptions = await receptionService.findReception(marche_id);
      if(Receptions)
      {
        res.status(201).json(Receptions);
      }
  ...
  async countReception(req, res) {
    try {
      const {marche_id} = req.params;

      const Receptions = await receptionService.countReception(marche_id);
      if(Receptions)
      {
        res.status(201).json(Receptions);
      }
